  ...
  if (Object.keys(SlackBlockUtil.getVoterStatusOptions()).includes(selectedVoterStatus)) {
    ...
    if (payload.actions[0].type === "button") {
      ...
      if (selectedVoterStatus === "REFUSED" || selectedVoterStatus === "SPAM") {
        await RedisApiUtil.setHash(redisClient, "slackBlockedUserPhoneNumbers", {[userPhoneNumber]: "1"});
        if (selectedVoterStatus === "SPAM") {
          await RedisApiUtil.setHash(redisClient, "twilioBlockedUserPhoneNumbers", {[userPhoneNumber]: "1"});
        }
      }
    ...
    await RedisApiUtil.deleteHashField(redisClient, "slackBlockedUserPhoneNumbers", userPhoneNumber);
    await RedisApiUtil.deleteHashField(redisClient, "twilioBlockedUserPhoneNumbers", userPhoneNumber);
  }
};
