...
SET twilio_phone_number=(
    SELECT from_phone_number FROM messages
    WHERE
        is_demo=threads.is_demo
        AND to_phone_number=threads.user_phone_number
        AND direction='OUTBOUND'
    ORDER BY twilio_receive_timestamp DESC limit 1
)
;
