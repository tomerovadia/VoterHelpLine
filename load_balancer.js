...
exports.selectSlackChannel = async (redisClient, entryPoint, stateName, isDemo = false) => {
  logger.debug("ENTERING LOADBALANCER.selectSlackChannel");
  logger.debug(`LOADBALANCER.selectSlackChannel: LoadBalancer given the following arguments: entryPoint: ${entryPoint}, stateName: ${stateName}, isDemo: ${isDemo}`);
  ...
  // to #lobby.
  if (!stateName) {
    logger.debug("LOADBALANCER.selectSlackChannel: U.S. state not provided, LoadBalancer returning null.");
    return null;
  }
  ...
  logger.debug(`LOADBALANCER.selectSlackChannel: Determined voterCounterKey: ${voterCounterKey}`);
  ...
  logger.debug(`LOADBALANCER.selectSlackChannel: Determined openPodsKey: ${openPodsKey}`);
  ...
  logger.debug(`LOADBALANCER.selectSlackChannel: Successfully found numVoters with voterCounterKey ${voterCounterKey} in Redis: ${numVoters}`);

  const openPods = await redisClient.lrangeAsync(openPodsKey, 0, 1000 /* max # of pods */);
  if (!openPods) {
    logger.error(`LOADBALANCER.selectSlackChannel: ERROR finding openPodsKey ${openPodsKey} in Redis: err || !openPods`);
    return null;
  }

  logger.debug(`LOADBALANCER.selectSlackChannel: Successfully found openPods with openPodsKey ${openPodsKey} in Redis: ${JSON.stringify(openPods)}`);

  numVoters = parseInt(numVoters);

  const selectedPodNumber = numVoters % openPods.length;
  logger.debug(`LOADBALANCER.selectSlackChannel: selectedPodNumber = numVoters % openPods.length = ${numVoters} % ${openPods.length} = ${selectedPodNumber}`);

  const selectedChannelName = openPods[selectedPodNumber];
  logger.debug(`LOADBALANCER.selectSlackChannel: selectedChannelName = openPods[selectedPodNumber] = ${openPods}[${selectedPodNumber}] = ${selectedChannelName}`);

  logger.debug(`LOADBALANCER.selectSlackChannel: Updating Redis voterCounterKey ${voterCounterKey} from ${numVoters} to ${numVoters + 1}`);
  await redisClient.setAsync(voterCounterKey, numVoters + 1);

  logger.debug(`Exiting LOADBALANCER.selectSlackChannel with return value: ${selectedChannelName}`);
